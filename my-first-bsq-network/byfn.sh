...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.bsq.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.bsq.com
    docker cp -a orderer.bsq.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.bsq.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.bsq.com

    for PEER in peer0.org1.bsq.com peer1.org1.bsq.com peer0.org2.bsq.com peer1.org2.bsq.com; do
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    # remove the docker-compose yaml file that was customized to the bsq
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.bsq.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.bsq.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.org1.bsq.com`` & ``peer0.org2.bsq.com``).  Secondly, we point to
